...
void keyexpansion(uint8_t rkey[11 * 16], const uint8_t mkey[16])
{
    ...
    for (uint8_t r = 16, idx_rcon = 1; r < 11 * 16; r += 16, idx_rcon++)
    {
        rkey[r + 0] = rkey[r - 16] ^ sbox[rkey[r - 3]] ^ rcon[idx_rcon];
        rkey[r + 1] = rkey[r - 15] ^ sbox[rkey[r - 2]];
        rkey[r + 2] = rkey[r - 14] ^ sbox[rkey[r - 1]];
        rkey[r + 3] = rkey[r - 13] ^ sbox[rkey[r - 4]];

        rkey[r + 4] = rkey[r - 12] ^ rkey[r + 0];
        rkey[r + 5] = rkey[r - 11] ^ rkey[r + 1];
        rkey[r + 6] = rkey[r - 10] ^ rkey[r + 2];
        rkey[r + 7] = rkey[r - 9] ^ rkey[r + 3];

        rkey[r + 8]  = rkey[r - 8] ^ rkey[r + 4];
        rkey[r + 9]  = rkey[r - 7] ^ rkey[r + 5];
        rkey[r + 10] = rkey[r - 6] ^ rkey[r + 6];
        rkey[r + 11] = rkey[r - 5] ^ rkey[r + 7];

        rkey[r + 12] = rkey[r - 4] ^ rkey[r + 8];
        rkey[r + 13] = rkey[r - 3] ^ rkey[r + 9];
        rkey[r + 14] = rkey[r - 2] ^ rkey[r + 10];
        rkey[r + 15] = rkey[r - 1] ^ rkey[r + 11];
